...
(function() {
	...
	function network_error_handler(url) {
		return function(jqxhr, text, err) {
			...
			case 404:
				...
				break;
			default:
				var msg = '<p>Failed to retrieve data from ' + url + ': ' + err
						+ '</p>';
			}
			show_err_msg(msg);
		};
	...
	function append_path(prefix, s) {
		var l = prefix.length;
		var p = l > 0 && prefix[l - 1] == '/' ? prefix.substring(0, l - 1)
				: prefix;
		return p + '/' + s;
	}
	...
	function view_file_details(path, abs_path) {
		function show_block_info(blocks) {
			...
			for (var i = 0; i < blocks.length; ++i) {
				var item = $('<option value="' + i + '">Block ' + i
						+ '</option>');
				menus.append(item);
			}
			menus.change();
		...
		$.ajax({
			...
			"crossDomain" : true
				...
				function(data) {
					console.log("GET_BLOCK_LOCATION");
					console.log(data);
					...
					var download_url = 'http://10.10.108.72:50070/webhdfs/v1'
							+ abs_path + '?op=OPEN';
					...
					$('#file-info-preview').click(
							function() {
								...
								var url = offset > 0 ? download_url
										+ '&offset=' + offset : download_url;
								$.get(url, function(t) {
	...
	function init() {
		...
		dust.loadSource(dust
				.compile($('#tmpl-block-info').html(), 'block-info'));
